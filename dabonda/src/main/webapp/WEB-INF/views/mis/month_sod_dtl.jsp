...
function showDisp() {
	...
	$.ajax({
		//url: 'ajax/test.json',
		url: '/month_sod/dtl/month_sod_dtl.json',
		dataType: 'json',
		type : "get",
		...
		success: function(data) {
			...
			$.each(data, function(key, val) {
				...
				for(i = 0; i < val.length; i++) {
					...
					*/
					...
					trHtml += '<td align="right">' + (val[i].plan_amt / div).toFixed(1) + '</td>';
					trHtml += '<td align="right">' + (val[i].curr_amt / div).toFixed(1) + '</td>';
					trHtml += '<td align="right">' + (val[i].real_amt / div).toFixed(1) + '</td>';
			...
			trHtml2 = '<tr height="30" style="color:white;">';
			trHtml2 += '<td>&nbsp;</td>';
			trHtml2 += '<td>계</td>';
			trHtml2 += '<td align="right">' + (sum_plan_amt / div).toFixed(1) + '</td>';
