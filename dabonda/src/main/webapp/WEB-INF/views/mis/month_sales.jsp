...
function go_dtl(p_cd, p_nm) {
	location.href="/month_sales/dtl?province_cd=" + p_cd + "&province_nm=" + p_nm + "&date=${date}";
}
</script>
<script>
...
function showDisp() {
	...
	$.ajax({
		url: '/month_sales/month_sales.json',
		dataType: 'json',
		type : "get",
		data : {date : date},
		success: function(data) {
			...
			$.each(data, function(key, val) {
				...
				for(i = 0; i < val.length; i++) {
					...
					if(rankCnt == 9) {
						var avr1, trHtml1;
						avr1 = sum_rslt_amt / sum_plan_amt * 100;
						trHtml1 = '<tr height="27" bgcolor="#E4F7BA">';
						trHtml1 += '<td>&nbsp;</td>';
						trHtml1 += '<td>소계</td><td align="right">' + sum_plan_amt.toFixed(1).format() + '</td>';
						trHtml1 += '<td align="right">' + sum_curr_amt.toFixed(1).format() + '</td>';
						trHtml1 += '<td align="right">' + sum_rslt_amt.toFixed(1).format() + '</td>';
						trHtml1 += '<td align="right">' + avr1.toFixed(2) + '&percnt;</td>';
						trHtml1 += '</tr>';
...
<div class="table">
	<p align="center" style="font-weight:bold; font-size:12pt">
		<font color="#FF4500">월간</font> 매출 실적 순위
	</p>
	<form name="fm" method="POST" action="/month_sales">
		날짜 <input type="date" name="date" size="8" value="${date}" /> <input type="submit" value="조회" />
	</form>
	<div id="caption"></div>
