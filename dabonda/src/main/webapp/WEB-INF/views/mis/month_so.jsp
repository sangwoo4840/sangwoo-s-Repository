...
function go_dtl(p_cd, p_nm) {
	location.href="/month_so/dtl?province_cd=" + p_cd + "&province_nm=" + p_nm;
}
</script>
<script>
			...
			$.each(data, function(key, val) {
				...
				for(i = 0; i < val.length; i++) {
					...
					if(rankCnt == 9) {
						...
						trHtml2 = '<tr height="27">';
						trHtml2 += '<td>&nbsp;</td>';
						trHtml2 += '<td>소계</td><td align="right">' + sum_plan_amt.toFixed(1).format() + '</td>';
						...
						$('table tbody').find('tr:last').css("background-color" , "#E4F7BA");
					}
				}
				}
			});
